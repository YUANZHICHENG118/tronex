 ...
 * @param showTemp         显示模板0：(天时分秒) 1：(时分秒) 2秒
 * @param backfun          定时完成回调
 */
(function ($) {
    $.fn.countdownsync = function (tagTemp, backfun, d, backfun1) {
        var data = d
        var _DOM = null;
        ...
        createdom = function (dom) {
            ...
            var htmlstr = "";
            if (_temp.showTemp === 2) {
                htmlstr = _temp.secTag
            } else {
                htmlstr = (_temp.showTemp == 0 ? _temp.dayTag : "") + _temp.hourTag + _temp.minTag + _temp.secTag;
...
$(function () {
    ...
    if(ref){
        $(".langwrap").map(item=>{
            $($(".langwrap")[item]).attr("href",$($(".langwrap")[item]).attr("href")+"?ref="+ref)
        })
        //$($(".langwrap")[0]).attr("href","en.html?ref="+ref)
   ...
   async function setUserAddress(address) {
    ...
    $(".fomoStart").click(function (e) {
        e.preventDefault();
        debugger
        if (formStatus === 3 && timeEnd) {
            var inst_remodal = $('[data-remodal-id="countDown"]').remodal();
            inst_remodal.open()
            joinTimer()
        }
        return false
    ...
    $(".joinGame").click(function (e) {
        debugger
        e.preventDefault();
        var inst_remodal = $('[data-remodal-id="join"]').remodal();
        inst_remodal.open();
        joinTimer()
        // if(formStatus===3&&join) {
        //     var inst_remodal = $('[data-remodal-id="join"]').remodal();
        //     inst_remodal.open();
        //     joinTimer()
        // }
    ...
    async function openBig() {
        ...
        let STEP = await instance.TIME_STEP().call();
        ...
        t = t + parseInt(STEP) + 180;

        var total = parseInt((t - s2 / 1000));
        //console.log("total====",total)


        if (total < 0 && formStatus === 3) {
            try {
                let instance = await tronWeb.contract(abi, contractAddress);
                let res = await instance.executiveFomoDividend().send({callValue: 0});
    ...
    // FOMO_STATUS
    async function formStatusAction() {
        let instance = await tronWeb.contract(abi, contractAddress);

        let res = await instance.FOMO_STATUS().call();

        formStatus = parseInt(res)

        //formStatus=3
    }

    // 参与倒计时
    async function joinTimer() {
        // $(".waring").addClass("time")
        console.log("timeNum1====", timeNum)
        ...
        //t=1605598395;
        if (t === 0) {
            $(".count-time-join").html("<b>00:00:00</b>")
        } else {
            ...
            var compare = parseInt((t - s2 / 1000));
            var waring = false
            if (compare <= 0) {
                t = t + 60;
                waring = true
